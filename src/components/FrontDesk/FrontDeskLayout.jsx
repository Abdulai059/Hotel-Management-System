  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        path: "/dashboard/frontdesk/housekeeping-audit",
        icon: ClipboardCheck,
      },
    ],
  },
    ...
    <div className="flex h-screen overflow-hidden bg-gray-50">
      ...
      <div className="flex min-h-screen flex-1 flex-col transition-all duration-300">
        <header className="flex h-14 items-center justify-between border-b border-gray-200 bg-stone-800 px-4 shadow-md sm:h-16 sm:px-6">
          <button onClick={() => setIsMobileMenuOpen(true)} className="shrink-0 lg:hidden">
...
function Sidebar({ isMobileMenuOpen, setIsMobileMenuOpen, isExpanded, setIsExpanded }) {
  const navigate = useNavigate();

  return (
    <aside
      className={`fixed inset-y-0 left-0 z-50 flex flex-col border-r border-gray-200 bg-white transition-all duration-300 ease-in-out lg:static lg:translate-x-0 ${
        isMobileMenuOpen ? "translate-x-0" : "-translate-x-full"
      } ${isExpanded ? "w-64" : "w-16"}`}
    >
      <div
        className={`flex items-center gap-4 bg-sky-200 p-[7.5px] shadow-sm transition-all duration-300 ${
      ...
      >
        ...
        >
          <h1 className="text-sm font-black tracking-tight whitespace-nowrap text-slate-900">GLOBAL DREAM HOTEL</h1>
          <div className="flex items-center gap-2">
            <span className="h-2 w-2 animate-pulse rounded-full bg-emerald-500" />
            <p className="text-[11px] font-semibold tracking-widest whitespace-nowrap text-slate-500 uppercase">
              Frontdesk Live
            </p>
          </div>
        </div>

        <button onClick={() => setIsMobileMenuOpen(false)} className="ml-auto shrink-0 lg:hidden">
      ...
      <button
        ...
        className="mt-0 hidden items-center justify-center bg-[#444444] p-2 font-bold text-gray-100 transition-all duration-300 hover:bg-[#555555] lg:flex"
      >
        {isExpanded ? (
          <div className="flex w-full">
            <ChevronsLeft strokeWidth={2.5} size={28} className="ml-auto transition-transform duration-300" />
          </div>
        ) : (
          <Menu strokeWidth={2.5} size={22} className="transition-transform duration-300" />
        )}
      </button>
      ...
      <nav className="mt-6 flex flex-1 flex-col gap-6 overflow-y-auto px-2 pb-12">
        {links.map(({ group, color, items }, index) => (
          <div key={group}>
            {isExpanded && (
              <div className="mb-2 flex items-center gap-2 px-4 transition-opacity duration-300">
            ...
            {isExpanded && index !== links.length - 1 && (
              <div className="my-4 border-t border-dashed border-gray-300 transition-opacity duration-300" />
            )}
          </div>
        ))}
      </nav>
      ...
      <button
        ...
        className="mx-auto mb-6 flex items-center justify-center gap-2 rounded-md bg-gradient-to-r from-sky-600 to-sky-700 px-3 py-3 font-bold text-yellow-300 transition-all duration-300 hover:from-sky-700 hover:to-sky-800"
      >
        <HiArrowLeftEndOnRectangle size={22} className="shrink-0" />
        {isExpanded && <span className="text-sm whitespace-nowrap">Back to Dashboard</span>}
      </button>
    </aside>
  );
}
