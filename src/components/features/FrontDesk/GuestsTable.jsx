...
const Flag = ({ show, label, color }) =>
  show ? <span className={`rounded-full px-3 py-0.5 text-xs font-medium ${color}`}>{label}</span> : null;
        ...
        <table className="w-full">
          <thead className="border-b border-gray-200 bg-gray-50">
            <tr>
              ...
              <th className="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase">services Flags</th>
              <th className="uppercasetext-left px-6 py-4 text-xs font-semibold text-gray-600 uppercase">Status</th>
          ...
          <tbody className="divide-y divide-gray-100">
            {bookings.map((booking) => (
              <tr key={booking.id} className="transition hover:bg-gray-50">
                ...
                <td className="px-6 py-4">
                  <span
                    className={`inline-block rounded-md px-3 py-1 text-xs font-semibold uppercase ${STATUS_STYLES[booking.status]}`}
                  >
                    {booking.status}
                  </span>
                </td>
